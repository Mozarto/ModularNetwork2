...
def add(lin):
    ...
    for i in temp:
        ...
        for j in range(calc):
            ...
            ind.neurons = copy.deepcopy(i.neurons)
            ind.first_neurons = copy.deepcopy(i.first_neurons)
            ind.delete_neuron(int(len(ind.neurons)*EV_RATE_NEU))
            ind.add_neuron(int((len(ind.neurons)*EV_RATE_NEU)/2))
