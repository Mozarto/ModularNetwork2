...
def main():
  ...
  def new_lin():
    ...
    answers = [
        entry_time, entry_size, entry_zoom, entry_food, entry_tree, entry_name
    ]

    accept_btn = Button(text="Start",
                        master=frame,
                        command=partial(set_sim, root, answers),
                        padx=30)
    decline_button = Button(text="Back",
                            master=frame,
                            command=lambda: main_frame.tkraise(),
                            padx=30)
  ...
  btn_new = Button(text="New lineage",
                   master=main_frame,
                   command=new_lin,
                   pady=10,
                   padx=30)
  ...
  btn_load = Button(text="Load lineage",
                    master=main_frame,
                    command=new_lin,
                    pady=10,
...
def set_sim(root, answers):
  ...
  screen_size_surfaces = (screen_size[0] / 4 - screen_border * 2,
                          screen_size[1] / 4 - screen_border * 2)

  square_size = 10
  view = View(screen_size_surfaces, square_size)

  lin = Lineage(view.squares, square_size, int(answers[0].get()),
                int(answers[2].get()), int(answers[1].get()))
  ...
  for ind in lin.individuals:
    values = {
        "lineage":
        int(
            database_get("lineages ORDER BY id DESC LIMIT 1", ["id"],
                         "Database_classes/modular_network.db")[0][0]),
        "score":
        ind.score,
        "distance_travelled":
        ind.distance_travelled
    }
    insert_individuals(values, "Database_classes/modular_network.db")
    for n in ind.first_neurons:
      values = {
          "id_ind":
          ind.first_neurons[n].ID,
          "individual":
          int(
              database_get("lineages ORDER BY id DESC LIMIT 1", ["id"],
                           "Database_classes/modular_network.db")[0][0]),
          ...
          1,
          "to_neuron":
          ind.first_neurons[n].to.ID,
          "threshold":
          ind.first_neurons[n].threshold,
          "to_depolarization_rate":
          ind.first_neurons[n].to_depolarization_rate,
          "repolarization":
          ind.first_neurons[n].repolarization,
          "to_depolarization":
          ind.first_neurons[n].to_depolarization
      }
    ...
    for n in ind.neurons:
      if type(ind.neurons[n].to) == str:
        values = {
            ...
            ind.neurons[n].to,
            "threshold":
            ind.neurons[n].threshold,
            "to_depolarization_rate":
            ind.neurons[n].to_depolarization_rate,
            "repolarization":
            ind.neurons[n].repolarization,
            "to_depolarization":
            ind.neurons[n].to_depolarization
        }
      else:
        values = {
            "id_ind":
